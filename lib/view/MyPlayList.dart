          ...
          children: [
            FutureBuilder(
                ...
                builder: (BuildContext context, AsyncSnapshot snapshot) {
                  ...
                  //스냅샷 정보가 없으면 보라색 동그라미

                  case ConnectionState.waiting:
                  return Center(child: CircularProgressIndicator(
                        valueColor: new AlwaysStoppedAnimation<Color>(Colors.deepPurple),
                      ));
                  //스냅샷 정보를 불러오는 중이면 보라색 동그라미
...
class _ItemsState extends State<Items> {
  ...
  // Audio audio = Audio('');
  // void audioUpdate(){
  //   List<dynamic>listItem = playlistItems();
  //   for(int i = 0; i < listItem.length; i++) {
  //     audio.updateMetas(
  //       player: assetsAudioPlayer,
  //       //add the player if the audio is actually played
  //       title: "My new title",
  ...
  //
  //         List<Audio> audioItems = [];
  //         audioItems.add(Custom.musicData[j]['path']);
  //         //매칭완료된 곡정보까지 들어간 선택곡 정보리스트
  //       }
  //     }
  //   return audioItems;
